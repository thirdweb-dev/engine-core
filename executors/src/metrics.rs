...
use prometheus::{HistogramOpts, Registry, Encoder, TextEncoder, register_histogram_vec_with_registry, HistogramVec};
use std::sync::Arc;
...
impl ExecutorMetrics {
    ...
    pub fn new(registry: &Registry) -> Result<Self, prometheus::Error> {
        ...
        let transaction_queued_to_confirmed_duration = register_histogram_vec_with_registry!(
            ...
            ).buckets(vec![0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0, 120.0, 300.0, 600.0, 1800.0]),
            &["executor_type", "chain_id"],
            registry
        )?;
...
pub fn record_transaction_queued_to_sent(executor_type: &str, chain_id: u64, duration_seconds: f64) {
    let metrics = get_metrics();
    metrics.transaction_queued_to_sent_duration
        .with_label_values(&[executor_type, &chain_id.to_string()])
        .observe(duration_seconds);
}

pub fn record_transaction_queued_to_confirmed(executor_type: &str, chain_id: u64, duration_seconds: f64) {
    let metrics = get_metrics();
    metrics.transaction_queued_to_confirmed_duration
        .with_label_values(&[executor_type, &chain_id.to_string()])
        .observe(duration_seconds);
}
...
pub fn record_eoa_job_processing_time(chain_id: u64, duration_seconds: f64) {
    let metrics = get_metrics();
    metrics.eoa_job_processing_duration
...
mod tests {
    ...
    fn test_custom_metrics_registry() {
        ...
        let custom_metrics = ExecutorMetrics::new(&custom_registry).expect("Should create custom metrics");
        ...
        let mut buffer = Vec::new();
        encoder.encode(&metric_families, &mut buffer).expect("Should encode metrics");
