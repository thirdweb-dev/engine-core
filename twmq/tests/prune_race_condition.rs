...
async fn test_prune_race_condition_two_workers() {
    ...
    tracing::warn!("This test reproduces the race where pruning deletes metadata while job is active");
    ...
    while iteration < max_iterations {
        ...
        if iteration % 100 == 0 {
            tracing::info!("Iteration {}/{}, race detected: {}", 
                iteration, max_iterations, RACE_DETECTED.load(Ordering::SeqCst));
        }
    ...
    if final_race_detected {
        tracing::error!("⚠️  UNEXPECTED: Race condition detected after {} iterations!", iteration);
        ...
        tracing::info!("✅ SUCCESS: No race detected after {} iterations", iteration);
        tracing::info!("   The fix is working correctly!");
        tracing::info!("   Pruning now checks if a job is active/pending/delayed before deleting metadata.");
        tracing::info!("   This prevents the race where Worker 1's pruning deletes Worker 2's active job metadata.");
    }
}
