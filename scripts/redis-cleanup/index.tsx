...
class EoaRedisCleanup {
  ...
  private async processBatch(keys: string[]): Promise<void> {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (result?.[0] === null) { // No Redis error
        const [status, completedAt] = result[1] as [string | null, string | null];
        ...
        if (key && key.includes(':')) {
          ...
          this.log(`📋 ${txData.id} | status: ${txData.status || 'null'} | completed_at: ${txData.completedAt || 'null'}`);
  ...
  private shouldClean(tx: TxData): boolean {
    ...
    if (tx.status !== 'confirmed' && tx.status !== 'failed') {
      this.log(`🛡️  SAFETY: Skipping tx ${tx.id} - status not explicitly confirmed/failed (status: ${tx.status})`);
      return false;
    }
    ...
    if (isNaN(completedMs) || completedMs <= 0) {
      this.log(`🛡️  SAFETY: Skipping tx ${tx.id} - invalid timestamp: ${tx.completedAt}`);
      return false;
    }
    ...
    if (completedMs > oneMinuteAgo) {
      this.log(`🛡️  SAFETY: Skipping tx ${tx.id} - completed too recently (less than 1 minute ago)`);
      return false;
    }

    this.log(`✅ SAFE TO CLEAN: ${tx.id} (${tx.status}) - completed ${new Date(completedMs).toISOString()}`);
    return true;
  }
  ...
  private async cleanTransaction(tx: TxData): Promise<void> {
    try {
      ...
      if (tx.status !== 'confirmed' && tx.status !== 'failed') {
        this.log(`🛡️  SAFETY: Double-check failed for ${tx.id} - aborting cleanup`);
        this.stats.skipped++;
        return;
      }
      ...
      if (CONFIG.dryRun) {
        ...
        this.log(`🔍 [DRY RUN] Would clean: ${tx.id} (${tx.status}) - ${keysToDelete.length} keys`);
        this.stats.cleaned++;
        return;
      }

      // FINAL SAFETY CONFIRMATION
      this.log(`⚠️  DELETING: ${tx.id} (${tx.status}) - ${keysToDelete.length} keys`);
      ...
      if (results?.every(([err]) => err === null)) {
        this.log(`✅ Cleaned: ${tx.id} (${tx.status}) - deleted ${keysToDelete.length} keys`);
        this.stats.cleaned++;
      } else {
        this.stats.errors++;
  ...
  private async batchCleanTransactions(transactions: TxData[]): Promise<void> {
    ...
    try {
      ...
      this.log(`🚀 BATCH DELETE: ${validTxIds.length} transactions (${totalKeys} keys)...`);
      ...
      if (results?.every(([err]) => err === null)) {
        ...
        this.log(`✅ BATCH CLEANED: ${validTxIds.length} transactions in ${duration}ms (${(validTxIds.length / duration * 1000).toFixed(1)} tx/sec)`);
      } else {
        // Count how many failed
        const failedCount = results?.filter(([err]) => err !== null).length || 0;
        this.stats.errors += failedCount;
        this.stats.cleaned += (validTxIds.length - failedCount);
        this.log(`⚠️ BATCH PARTIAL: ${validTxIds.length - failedCount} cleaned, ${failedCount} failed in ${duration}ms`);
      }
  ...
  private reportProgress(): void {
    ...
    if (this.stats.scanned % (CONFIG.progressInterval * 5) === 0 && this.stats.scanned > 0) {
      this.log(`🎯 Milestone: ${this.stats.scanned.toLocaleString()} transactions processed`);
    }
  }
}
    ...
    <Box flexDirection="column" width="100%">
      ...
      <Box borderStyle="round" borderColor="green" paddingX={1}>
        <Text>
          ...
          <Text> | </Text>
          <Text color="yellow">{CONFIG.dryRun ? 'Would Clean' : 'Cleaned'}: </Text>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const runCleanup = async () => {
      ...
      cleaner.log(`   DRY RUN: ${CONFIG.dryRun} ${CONFIG.dryRun ? '✅ SAFE' : '⚠️ WILL DELETE DATA'}`);
      cleaner.log(`   Redis URL: ${CONFIG.redisUrl.replace(/:[^:@]*@/, ':***@')}`);
      ...
      if (!CONFIG.dryRun) {
        ...
        cleaner.log('⚠️ Only transactions with status="confirmed" OR status="failed"');
        cleaner.log('⚠️ AND completed_at timestamp AND older than 1 minute will be deleted.');
        cleaner.log('🛡️ Multiple safety checks are in place, but this is still irreversible.');
      }
      ...
      try {
        ...
        if (CONFIG.dryRun) {
          cleaner.log('💡 DRY RUN - Set CONFIG.dryRun = false to actually delete');
        } else {
          cleaner.log('✅ CLEANUP COMPLETED - Data has been permanently deleted');
        }
